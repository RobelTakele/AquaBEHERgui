...
app_server <- function(input, output, session) {
  ...
  plot_PET <- eventReactive(input$PET_plotButton, {
    ...
    if (input$PETplot_typ == 'scatter') {
      locPETplt <- plotly::plot_ly(data = pet.dF, type =  'scatter',   #input$PETplot_typ,
                                   mode = 'line', x = x.date,  y =  pet.dF[,input$PETplot_y],
                                  # color = I(input$PETplot_color),
                                   alpha = 1)
    }else {
...
#                     filename = paste0(out.dir, "/",  out.file, "_",
...
#                     overwrite = TRUE)
    ...
    spPEToutputDir <- "Output/Data/PET/"


    terra::writeCDF(PET.rasBRK,
                    filename = paste0(spPEToutputDir, "/",  out.file, "_",
                                      Res, "_", method, "_", DateStart,"_T_",
                                      DateEnd, ".nc"),
                    varname = "PET",
                    longname = paste0("Potential Evapotranspiration [", method, "]"),
  ...
  plot_SWB <- eventReactive(input$SWB_plotButton, {
    ...
    if (input$SWBplot_typ == 'scatter') {
      ...
      locSWBplt <- plotly::plot_ly(data = locSWB.dF, type = input$SWBplot_typ,
                                   x = SWB.x.date, mode = 'lines',
                                   y = locSWB.dF[ ,input$SWBplot_y],
                                  # color = I(input$SWBplot_color),
                                   alpha = 1)
    }else {
